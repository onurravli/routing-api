...
> {
	public async handleRequest(
				...
				switch (protocolStr.toLowerCase()) {
					...
					default:
						return {
							...
							detail: `Invalid protocol specified. Supported protocols: ${JSON.stringify(
		...
		for (const subRoute of route) {
			...
			const pools =
				subRoute.protocol == Protocol.V2 ? subRoute.route.pairs : subRoute.route.pools;
			...
			for (let i = 0; i < pools.length; i++) {
				...
				if (nextPool instanceof Pool) {
					...
					curRoute.push({
						type: 'v2-pool',
						address: v2PoolProvider.getPoolAddress(nextPool.token0, nextPool.token1)
							.poolAddress,
						tokenIn: {
							chainId: tokenIn.chainId,
							decimals: tokenIn.decimals.toString(),
