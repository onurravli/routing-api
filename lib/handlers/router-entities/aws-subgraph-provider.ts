...
const LOCAL_POOL_CACHE_KEY = (chainId: ChainId, protocol: Protocol) =>
...
export class AWSSubgraphProvider<TSubgraphPool extends V2SubgraphPool | V3SubgraphPool> {
	...
	public async getPools(): Promise<TSubgraphPool[]> {
		...
		const cachedPools = POOL_CACHE.get<TSubgraphPool[]>(
			LOCAL_POOL_CACHE_KEY(this.chain, this.protocol),
		);
		...
		const pools = await cachePoolsFromS3<TSubgraphPool>(
			s3,
			this.bucket,
			this.baseKey,
			this.chain,
...
export const cachePoolsFromS3 = async <TSubgraphPool>(
	...
	try {
		...
		log.error(
			{ bucket, key, err },
			`Failed to get pools from S3 for ${protocol} on chain ${chainId}`,
		);
	...
	if (!poolsBuffer) {
		throw new Error(
			`Could not get subgraph pool cache from S3 for protocol ${protocol} on chain ${chainId}`,
		);
	}
	...
	log.info(
		{ bucket, key },
		`Got subgraph pools from S3 for protocol ${protocol} on ${chainId}. Num: ${pools.length}`,
	);
...
export class V3AWSSubgraphProvider
	extends AWSSubgraphProvider<V3SubgraphPool>
	implements IV3SubgraphProvider
{
	...
	public static async EagerBuild(
		bucket: string,
		baseKey: string,
		chainId: ChainId,
	): Promise<V3AWSSubgraphProvider> {
		const s3 = new S3();
...
export class V2AWSSubgraphProvider
	extends AWSSubgraphProvider<V2SubgraphPool>
